...
SELECT 
 ...
 v_first_res_req.solicitor_list "Requested By"
...
WHERE 
SOURCE = 6014 AND Category_Code  = 6015) AS T_1 ON RECORDS_PROSPECT.ID = T_1.PROSPECT_ID 
LEFT OUTER JOIN (SELECT 
 RATINGS.*, 
TABLEENTRIES.LongDESCRIPTION, 
TABLEENTRIES.SHORTDESCRIPTION
...
WHERE 
SOURCE = 5912 AND Category_Code  = 5967  ) AS T_2 ON RECORDS_PROSPECT.ID = T_2.PROSPECT_ID
left join (select t1.*,
...
)v_first_res_req on v_first_res_req.RECORDS_ID = RECORDS.ID
