...
pp.classification,
pp.status,
'PSC' + '-' + replace(convert(nvarchar(MAX), GETDATE(), 110),'-','')+'-' +  CAST(ROW_NUMBER() over (order by r.id) as varchar)  as new_action_import_ID_ACImpID,
'Task/Other' as new_action_category_ACCat,
'Prospect Status Change' as new_action_type_ACType,
convert(nvarchar(MAX), GETDATE(), 101) as new_action_date_ACDate,
'N' as new_action_is_complete_ACComplete,

'PSCP' + '-' + replace(convert(nvarchar(MAX), GETDATE(), 110),'-','')+'-' +  CAST(ROW_NUMBER() over (order by r.id) as varchar)  as new_action_prior_attribute_import_id_ACAttrImpID,
'Prospect Status-Prior' as new_action_prior_status_category_ACAttrCAT,
a2.old_action_current_attribute_ACAttrDesc as new_action_prior_attribute_ACAttrDesc,
'PSCC' + '-' + replace(convert(nvarchar(MAX), GETDATE(), 110),'-','')+'-' +  CAST(ROW_NUMBER() over (order by r.id) as varchar) as new_action_current_attribute_import_id_ACAttrImpID,
'Prospect Status-Current' as new_action_prior_status_category_ACAttrCAT,
pp.status as new_action_current_attribute_ACAttrDesc,
'00001-593-0000511720' as New_Action_Solicitor_Import_ID_ACSolImpID,--This is the Import ID for Adrian Jarrett


a2.old_action_import_ID_ACImpID,
a2.old_action_status_ACStatus,
case when a2.old_action_import_ID_ACImpID is null then null else 'Y' end as old_action_is_complete_ACComplete,
case when a2.old_action_import_ID_ACImpID is null then null else 
	case when a2.old_action_current_attribute_ACAttrDesc <> pp.status then convert(nvarchar(MAX), GETDATE(), 101)
		...
		end end as old_action_completed_on_ACComplete_Date,
convert(nvarchar(MAX),a2.old_action_date_ACDate,101) as old_action_date_ACDate,



case when a2.old_action_prior_attribute_import_id_ACAttrImpID is null then null else 'PSCD' + '-' + replace(convert(nvarchar(MAX), GETDATE(), 110),'-','')+'-' +  CAST(ROW_NUMBER() over (order by r.id) as varchar) end as old_action_days_attribute_import_id_ACAttrImpID ,
case when a2.old_action_prior_attribute_import_id_ACAttrImpID is null then null else 'Prospect Status-# Days' end as old_action_days_category_ACAttrCat,

DATEDIFF(DAY,a2.old_action_date_ACDate,GETDATE()) as old_action_days_attribute_ACAttrDesc
